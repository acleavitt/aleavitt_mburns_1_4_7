...
def modify_image(origional_image): 
    # Open the files in the same directory as the Python script
    #directory = os.path.dirname(os.path.abspath(__file__))  
    #picture_file = os.path.join(directory, origional_image)
    
    

    # Open and show the picture image in a new Figure window
    #origional_image = PIL.Image.open(picture_file)
    fig, axes = plt.subplots(1, 2)
    axes[0].imshow(origional_image, interpolation='none')
    width, height = origional_image.size
    percentage_top = int(height*.05)
    percentage_side = int(width*.05)


    # Display picture in second axes and set window to the right eye
    axes[1].imshow(origional_image, interpolation='none')
    ...
    origional_image.paste(earth_small, ((width-100),(height-100)), mask=earth_small) 
    # Display
    fig3, axes3 = plt.subplots(1, 2)
    axes3[0].imshow(origional_image, interpolation='none')
    axes3[1].imshow(origional_image, interpolation='none')
    ...
    origional_image.paste(abstract_small, (percentage_side, percentage_top), mask=0) 

    #Paste absract image in top right corner
    origional_image.paste(abstract_small, (((width-percentage_side)-100), percentage_top), mask=0) 
    
    #paste absract image in bottom left corner
    origional_image.paste(abstract_small, (percentage_side, ((height-percentage_top)-100)), mask=0) 
